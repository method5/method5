...
begin
	...
	loop
		...
		--Create error if the user is not found
		if v_table_name is null then

			v_error_message := v_error_message || 'The user '||v_users(i)||' was not found.  That user must '||
				'first exist on '||sys_context('userenv', 'server_host')||' before it can be copied to other servers.' || chr(10);
		--Create error if the user does not have a DES hash.
		--A DES hash is still required for some databases.
		elsif v_des_password_hash is null then

			v_error_message := v_error_message || 'The user '||v_users(i)||' does not have a DES password hash.'||
				'  Databases with SEC_CASE_SENSITIVE_LOGON=false still require a DES password hash.' || chr(10);
		--Only process this user if there is no error.
		else
	...
	--Raise the error, if any, after the processing is complete.
	--This way one error won't stop everyone else.
